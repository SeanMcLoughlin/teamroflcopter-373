...
#else
#define SPI_DC_HIGH()           Chip_GPIO_SetPinState(LPC_GPIO, 0, _dc, 1)
#define SPI_DC_LOW()            Chip_GPIO_SetPinState(LPC_GPIO, 0, _dc, 0)
#define SPI_CS_HIGH()           Chip_GPIO_SetPinState(LPC_GPIO, 0, _cs, 1)
#define SPI_CS_LOW()            Chip_GPIO_SetPinState(LPC_GPIO, 0, _cs, 0)
#endif
...
#else
#define SSPI_MOSI_HIGH()        Chip_GPIO_SetPinState(LPC_GPIO, 0, _mosi, 1)
#define SSPI_MOSI_LOW()         Chip_GPIO_SetPinState(LPC_GPIO, 0, _mosi, 0)
#define SSPI_SCK_HIGH()         Chip_GPIO_SetPinState(LPC_GPIO, 0, _sclk, 1)
#define SSPI_SCK_LOW()          Chip_GPIO_SetPinState(LPC_GPIO, 0, _sclk, 0)
#define SSPI_MISO_READ()        Chip_GPIO_GetPinState(LPC_GPIO, 0, _miso)
#endif
...
#else
#define SPI_DEFAULT_FREQ         60000000
#endif
// TODO !!!!!!!!!!!!!!!!!!!!!!!! ????????
...
#endif
{
    ...
    // Control Pins
    Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, _dc);
    Chip_GPIO_SetPinState(LPC_GPIO, 0, _dc, 0);
    Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, _cs);
    Chip_GPIO_SetPinState(LPC_GPIO, 0, _cs, 1);
    /*
    pinMode(_dc, OUTPUT);
    ...
    // Software SPI
    if(_sclk >= 0){
    	Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, _mosi);
    	Chip_GPIO_SetPinState(LPC_GPIO, 0, _mosi, 0);
    	Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, _sclk);
        Chip_GPIO_SetPinState(LPC_GPIO, 0, _sclk, 1);
        /*
        ...
        if(_miso >= 0){
        	Chip_GPIO_SetPinDIRInput(LPC_GPIO, 0, _miso);
            //pinMode(_miso, INPUT);
        }
    }
    ...
    if (_rst >= 0) {
    	...
    	Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, _rst);
    	Chip_GPIO_SetPinState(LPC_GPIO, 0, _rst, 1);


    	//pinMode(_rst, OUTPUT);
        //digitalWrite(_rst, HIGH);
        delay(100);
        Chip_GPIO_SetPinState(LPC_GPIO, 0, _rst, 0);
